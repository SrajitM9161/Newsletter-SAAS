...
import { Pen, X, Delete } from "lucide-react";
import { Button } from "@/components/ui/button";
import toast from "react-hot-toast";
import { useRouter } from "next/navigation";
import { getEmails, deleteEmail } from "@/actions/get.emails";
...
const Write = () => {
    ...
    const deleteHanlder = async (id: string) => {
        const confirmation = window.confirm("Do you want to permanently delete this email?");
        if (confirmation) {
            try {
    ...
    return (
        <div className="w-full p-5 flex flex-wrap gap-6 relative">
            <div
                className="w-[200px] h-[200px] bg-slate-50 flex flex-col justify-center items-center rounded border cursor-pointer"
